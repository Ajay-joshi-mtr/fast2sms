...
use Ajayjoshi\Fast2sms\Exception\InvalidMethodException;
...
//            $this->response = $promise->wait()->getBody()->getContents();
    ...
    {
        $default_acc = config('fast2sms-api.default', null);
        if ($default_acc) {
    ...
    protected function loadCredentialsFromConfig()
    {
        ...
        $config_name = 'fast2sms-api.' . $gateway;       
    ...
    private function addCountryCode($mobile)
    {
        if (!$this->country_code) {
            $this->country_code = config('fast2sms-api.country_code', '91');
        }
        if (is_array($mobile)) {
